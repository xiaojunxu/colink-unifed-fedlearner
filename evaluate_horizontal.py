...
class TestFedavgTrain():
    def __init__(self):
        ...
        for i in range(client_num-1):
            self._fl_cluster["followers"].append({
            "name": "follower_"+str(i),
            "address": "0.0.0.0:"+str(20051+i)
        })

        self._batch_size = config['training_param']['batch_size']
        self._epochs = config['training_param']['epochs']
        self._steps_per_sync = config['training_param']['steps_per_sync']
        self._l_eval_result = None
        self._f_eval_result = []
        for i in range(client_num-1):
            self._f_eval_result.append(None)

    def _train_leader(self):
        model = self.models[0]
        if config['dataset'] == 'reddit':
            self._l_eval_result = train_from_keras_model(model,
                                                        all_test_seq[:,:-1],
                                                        all_test_seq[:,1:],
                                                        train_data_seq[all_users[0]][:,:-1],
                                                        train_data_seq[all_users[0]][:,1:],
    ...
    def _train_follower_i(self, i):
        model = self.models[i+1]
        if config['dataset'] == 'reddit':
            self._f_eval_result[i] = train_from_keras_model(model,
                                                        all_test_seq[:,:-1],
                                                        all_test_seq[:,1:],
    ...
    def test_train(self):
        ...
        for i in range(client_num):
            self.models.append(create_model(num_class, input_len, type))
        
        threads = []
        threads.append(threading.Thread(target=self._train_leader))
        for i in range(client_num-1):
            threads.append(threading.Thread(target=self._train_follower_i, args=(i,)))
